...
public class SMS
{
	...
	private static int _send(string content, string to)
	{
		if (status != 0)
		{
			...
			if (status != 0)
			{
				Out.println("CANNOT SEND SMS " + content + " WHEN SENDING " + _content);
				return -1;
			}
		}
		_content = content;
		...
		if (j == 500)
		{
			Out.LogWarning("TOO LONG FOR SEND SMS " + content);
			status = 0;
		}
		else
		{
			Out.Log("Send SMS " + content + " done in " + j * 5 + "ms");
		}
		return _result;
	}
