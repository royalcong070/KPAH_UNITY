...
public class WindowInfoScr : MyScreen
{
	...
	public void setSelectTab()
	{
		...
		else if (charWearing != null && charWearing.ID != Canvas.gameScr.mainChar.ID)
		{
	...
	public void setCmdCenter()
	{
		...
		case 1:
		{
			...
			if (charWearing != null && charWearing.ID != Canvas.gameScr.mainChar.ID)
			{
				...
				left = new Command("Thong tin");
				left.action = action11;
			}
			break;
		}
		case 2:
		...
		case 4:
			...
			if (charWearing != null && charWearing.ID != Canvas.gameScr.mainChar.ID)
			{
				ActionPerform action7 = delegate
				{
					WindowInfoScr.gI().setInfo(0, false, new sbyte[1] { 1 });
					WindowInfoScr.gI().name = "Trang bi";
				};
				left = new Command("Trang bi");
				left.action = action7;
			}
			break;
		case 5:
	...
	public void doViewInfoItemTradeup()
	{
		...
		if (num2 == 4)
		{
			...
			{
				showTradeMoneyInfo(num - 3, false, x1, y1);
			}
			return;
		}
		if (num <= 2)
	...
	private string getTradeMoneyName(int moneyType)
	{
		switch (moneyType)
		{
		case 0:
	...
	private long getTradeMoneyValue(bool isMine, int moneyType)
	{
		switch (moneyType)
		{
		case 0:
			return isMine ? Canvas.gameScr.mainChar.tXuTrade : Canvas.gameScr.mainChar.rXuTrade;
		case 1:
			return isMine ? Canvas.gameScr.mainChar.tLuongTrade : Canvas.gameScr.mainChar.rLuongTrade;
		case 2:
			return isMine ? Canvas.gameScr.mainChar.tLuongKhoaTrade : Canvas.gameScr.mainChar.rLuongKhoaTrade;
		default:
			return 0L;
		}
	}

	private long getTradeMoneyAvailable(int moneyType)
	{
		switch (moneyType)
		{
		case 0:
			return Canvas.gameScr.mainChar.charXu;
		case 1:
			return Canvas.gameScr.mainChar.luong;
		case 2:
			return (Canvas.gameScr.mainChar.luongKhoa > 0) ? Canvas.gameScr.mainChar.luongKhoa : 0;
		default:
			return 0L;
		}
	}

	private string getTradeMoneyShort(long amount)
	{
		if (amount < 1000L)
		{
			return amount.ToString();
		}
		string[] array = new string[5] { string.Empty, "K", "M", "B", "T" };
		int num = 0;
		while (amount >= 1000L && num < array.Length - 1)
		{
			long num2 = amount % 1000L;
			amount /= 1000L;
			num++;
			if (amount < 100L && num2 > 0L)
			{
				return amount + "." + num2 / 100L + array[num];
			}
		}
		return amount + array[num];
	}

	private void showTradeMoneyInfo(int moneyType, bool isMine, int x1, int y1)
	{
		long tradeMoneyValue = getTradeMoneyValue(isMine, moneyType);
		long tradeReceiveAmount = MainChar.getTradeReceiveAmount(tradeMoneyValue);
		long num = tradeMoneyValue - tradeReceiveAmount;
		string tradeMoneyName = getTradeMoneyName(moneyType);
		string text = (!isMine) ? ("Doi phuong gui: " + Canvas.getMoneys(tradeMoneyValue) + " " + tradeMoneyName + "\nBan nhan: " + Canvas.getMoneys(tradeReceiveAmount) + " " + tradeMoneyName + "\nThue: " + Canvas.getMoneys(num) + " " + tradeMoneyName) : ("Ban gui: " + Canvas.getMoneys(tradeMoneyValue) + " " + tradeMoneyName + "\nDoi phuong nhan: " + Canvas.getMoneys(tradeReceiveAmount) + " " + tradeMoneyName + "\nThue: " + Canvas.getMoneys(num) + " " + tradeMoneyName);
		setShowTextNew(text, x1, y1);
	}

	private void inputTradeMoney(int moneyType)
	{
		long tradeMoneyValue = getTradeMoneyValue(true, moneyType);
		long tradeMoneyAvailable = getTradeMoneyAvailable(moneyType);
		ActionPerform ok = delegate
		{
			string text = Canvas.inputDlg.tfInput.getText();
			if (text.Equals(string.Empty))
			...
			try
			{
				...
				if (num < 0L)
				{
					Canvas.startOKDlg("Khong the nhap so am.");
				}
				else if (num > tradeMoneyAvailable)
				{
					Canvas.startOKDlg("Ban chi co the giao dich toi da " + Canvas.getMoneys(tradeMoneyAvailable) + " " + getTradeMoneyName(moneyType));
				}
				else if (num > 0L && MainChar.getTradeReceiveAmount(num) <= 0L)
				{
					Canvas.startOKDlg("So tien giao dich toi thieu la 10.");
				}
				else
				{
					Canvas.gameScr.gameService.sendTradeMoney((sbyte)moneyType, num);
					Canvas.endDlg();
				}
			}
			catch (Exception)
			{
				Canvas.startOKDlg("Co loi xay ra. Vui long chi nhap so.");
			}
		};
		Canvas.inputDlg.setInfo("Nhap " + getTradeMoneyName(moneyType), ok, TField.INPUT_TYPE_NUMERIC, 18, true);
		Canvas.inputDlg.tfInput.setText(tradeMoneyValue + string.Empty);
		Canvas.inputDlg.show();
	}
	...
	private void setTextCharInfo()
	{
		...
		MainChar mainChar = Canvas.gameScr.mainChar;
		if (charWearing != null && charWearing.ID != mainChar.ID)
		{
			charInfo = new string[11];
			charInfo[0] = "Nhan vat: " + charWearing.name;
			charInfo[1] = "Level: " + charWearing.level + "+" + getViewedPercentText(charWearing.lvpercent);
			charInfo[2] = "HP: " + charWearing.hp + "/" + charWearing.maxhp;
			charInfo[3] = "MP: " + charWearing.mp + "/" + charWearing.maxmp;
			charInfo[4] = "Tan cong: " + charWearing.getAttack();
			charInfo[5] = "Thu vat ly: " + charWearing.defend;
			charInfo[6] = "Thu ma phap: " + charWearing.defend_magic;
			charInfo[7] = "Chinh xac: " + charWearing.accurate;
			charInfo[8] = "Ne tranh: " + charWearing.dodge;
			charInfo[9] = "Bao kich: " + getBaoKichText(charWearing.baokich);
			charInfo[10] = "Chi mang: " + charWearing.critical;
			numH = charInfo.Length + 1;
			setLim();
			return;
		}
		/*
		Char viewedChar = getViewedCharInfoSource();
		if (charWearing != null && viewedChar != null && viewedChar.ID != mainChar.ID)
		{
			int num = getViewedInt(viewedChar.hp, charWearing.hp);
			int num2 = getViewedInt(viewedChar.maxhp, charWearing.maxhp);
			int num3 = getViewedInt(viewedChar.mp, charWearing.mp);
			int num4 = getViewedInt(viewedChar.maxmp, charWearing.maxmp);
			int num5 = getViewedInt(viewedChar.getAttack(), charWearing.getAttack());
			int num6 = getViewedInt(viewedChar.defend, charWearing.defend);
			int num7 = getViewedInt(viewedChar.defend_magic, charWearing.defend_magic);
			short num8 = getViewedShort(viewedChar.accurate, charWearing.accurate);
			short num9 = getViewedShort(viewedChar.dodge, charWearing.dodge);
			short num10 = getViewedShort(viewedChar.critical, charWearing.critical);
			short num11 = getViewedShort(viewedChar.baokich, charWearing.baokich);
			charInfo = new string[11];
			charInfo[0] = "NhÃ¢n váº­t: " + getViewedName(viewedChar);
			charInfo[1] = "Level: " + getViewedLevel(viewedChar) + "+" + getViewedPercent(viewedChar);
			charInfo[2] = "HP: " + num + "/" + num2;
			charInfo[3] = "MP: " + num3 + "/" + num4;
			charInfo[4] = "Táº¥n cÃ´ng: " + num5;
			charInfo[5] = "Thá»§ váº­t lÃ½: " + num6;
			charInfo[6] = "Thá»§ ma phÃ¡p: " + num7;
			charInfo[7] = "ChÃ­nh xÃ¡c: " + num8;
			charInfo[8] = "NÃ© trÃ¡nh: " + num9;
			charInfo[9] = "Báº¡o kÃ­ch: " + getBaoKichText(num11);
			charInfo[10] = "ChÃ­ máº¡ng: " + num10;
			charInfo[0] = "Nhan vat: " + getViewedName(viewedChar);
			charInfo[4] = "Tan cong: " + num5;
			charInfo[5] = "Thu vat ly: " + num6;
			charInfo[6] = "Thu ma phap: " + num7;
			charInfo[7] = "Chinh xac: " + num8;
			charInfo[8] = "Ne tranh: " + num9;
			charInfo[9] = "Bao kich: " + getBaoKichText(num11);
			charInfo[10] = "Chi mang: " + num10;
			numH = charInfo.Length + 1;
			setLim();
			return;
		}
	...
	private string getViewedPercentText(short percent)
	{
		return percent / 10 + "." + percent % 10 + "%";
	}

	private string getBaoKichText(short value)
	{
		if (value <= 0)
		{
			return "0";
		...
		else if (index[focusTab] == 8)
		{
			...
			else if (Canvas.isPointer(25 + xTr, 65 + hView - (newZise + 53) + yTr, 5 * (wCell + 12), newZise))
			{
				...
				else if (Canvas.isPointerClick)
				{
					...
					isPaintfc = false;
					trans = false;
					int num12 = (cmtoX + Canvas.px - 25 - xTr) / 34;
					selected = num12;
	...
	private void paintTrade(MyGraphics g)
	{
		...
		paintTradeMoneyCell(g, 0, "XU", Canvas.gameScr.mainChar.tXuTrade, true);
		paintTradeMoneyCell(g, 1, "L", Canvas.gameScr.mainChar.tLuongTrade, true);
		paintTradeMoneyCell(g, 2, "LK", Canvas.gameScr.mainChar.tLuongKhoaTrade, true);
		paintTradeMoneyCell(g, 3, "XU", Canvas.gameScr.mainChar.rXuTrade, false);
		paintTradeMoneyCell(g, 4, "L", Canvas.gameScr.mainChar.rLuongTrade, false);
		paintTradeMoneyCell(g, 5, "LK", Canvas.gameScr.mainChar.rLuongKhoaTrade, false);
	...
	private void paintTradeMoneyCell(MyGraphics g, int column, string label, long value, bool isMine)
	{
		int num = column * 33 - 17 + xTr;
		int num2 = 4 * 32 - 8 + yTr;
		string tradeMoneyShort = getTradeMoneyShort(isMine ? value : MainChar.getTradeReceiveAmount(value));
		MFont.smallFont.drawString(g, label, num + 17, num2 + 5, MFont.CENTER);
		MFont.smallFont.drawString(g, tradeMoneyShort, num + 17, num2 + 18, MFont.CENTER);
	}
			...
			if (MainChar.itemQuest != null)
			{
				...
				for (int num4 = 0; num4 < num3; num4++)
				{
					...
					if (MainChar.itemQuest[num4] > 0)
					{
						...
						{
							setShowTextNew(getInfoItemQuest(aa5), ii4 % numW * 18, ii4 / numW * 18);
						};
	...
	private void showMenuForPotion(int potionType)
	{
		...
		if (potionType < 14 || potionType >= 21)
		{
			...
			ActionPerform action4 = delegate
			{
				...
				{
					int ii = i;
					Command command4 = new Command("Phím số " + (7 + 2 * i));
					ActionPerform action3 = delegate
					{
					...
					else
					{
						...
						ActionPerform ok = delegate
						{
							try
							{
								...
								{
									ItemInInventory itemInInventory2 = (ItemInInventory)readyBuy.elementAt(bb);
									...
									{
										setShowText(item.getTemplateDescription(), selected % numW * 18, selected / numW * 18);
									}
								}
							}
							catch (Exception)
						...
						Canvas.inputDlg.setInfo("Số lượng: ", ok, TField.INPUT_TYPE_NUMERIC, 10, true);
						Canvas.inputDlg.show();
					}
				}
			}
